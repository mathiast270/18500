...
def find_beat_pos(in_list, beat_start, beat_end):
    ...
    for idx, ele in enumerate(in_list):
        print(ele[2])
        if(beats >= beat_start and not found_start):
            found_start_idx = idx
            found_start = True
        if(beats >= beat_end):
     ...
     for system in systems:
        ...
        for ele in listsing_:
            ...
            if(x_post  <= prev_x +5 and x_post >= prev_x - 5 ):
                continue
            singer_list.append((ele[0], ele[1],beat_list_singer[count_s] ))
            prev_x = x_post
            count_s += 1
        ...
        for ele in list_piano_2:
            ...
            count_piano_r += 1
        '''
     (idx1, idx2) = find_beat_pos(singer_list, 8, 15)
     print((idx1,idx2))
     cv2.rectangle(template, (singer_list[idx1][0], singer_list[idx1][1]), 
